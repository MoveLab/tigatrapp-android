package ceab.movelab.tigabib.workers;
...
import ceab.movelab.tigabib.ContProvValuesMissions;
...
Util.logInfo(TAG, "SyncData doWork");
    ...
    private void tryUploads() {
        ...
        if ( !PropertyHolder.isRegistered() ) {
            ...
            try {
                ...
                String token = FirebaseInstanceId.getInstance().getToken(); // ?? Revisar gettoken deprecated
...
Util.logInfo(TAG, "set property holder");
            ...
            Util.logCrashlyticsException(TAG + "Config JSON", e);
        }

        ContentResolver cr = context.getContentResolver();

        // try to get missions from server check last id on phone ??? DELETE CANDIDATE
        ...
        if ( c != null ) {
            ...
            while ( !c.isAfterLast() ) {
                ...
                if ( uploadResult > 0 ) {
                    ...
                    // if not latest version or if this is a deletion, then delete from phone
                    if (report.getLatestVersion() != 1 || report.getReportVersion() == -1 || report.getDeleteReport() == 1) {
